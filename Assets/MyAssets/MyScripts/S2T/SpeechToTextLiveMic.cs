...
public class SpeechToTextLiveMic : MonoBehaviour
{
	...
	void Start ()
	{
		...
		int minHertz;
		int maxHertz; 
		Microphone.GetDeviceCaps (null,out minHertz,out maxHertz);
		m_RecordingHZ = (int)Mathf.Lerp ((float)minHertz, (float)maxHertz, 0.5f);
		Debug.Log ("min hertz = " + minHertz + " maxherts = " + maxHertz + "  assigned hertz = " + m_RecordingHZ);
		Active = true;
	...
	private IEnumerator RecordingHandler ()
	{
		...
		if (m_Recording == null)
			textLabel.text = "Mic Fail";
		yield return null;      // let m_RecordingRoutine get set..
		textLabel.text = "Mic Good";
		if (m_Recording == null) {
			StopRecording ();
			yield break;
		}
